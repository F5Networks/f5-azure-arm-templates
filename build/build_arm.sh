...
for tmpl in $template_list; do
    ...
    for stack_type in $stack_list; do
        ...
        python -B '.\master_template.py' --template-name $tmpl --license-type BYOL --stack-type $stack_type --template-location "../experimental/$loc/$stack_type/BIGIQ/" --script-location "../experimental/$loc/$stack_type/"
    done
done
...
## BIGIP ARM Templates - Cluster (1nic)
python -B '.\master_template.py' --template-name cluster_1nic --license-type PAYG --stack-type new_stack --template-location '../supported/cluster/1nic/PAYG/' --script-location '../supported/cluster/1nic/'
python -B '.\master_template.py' --template-name cluster_1nic --license-type BYOL --stack-type new_stack --template-location '../supported/cluster/1nic/BYOL/' --script-location '../supported/cluster/1nic/'
