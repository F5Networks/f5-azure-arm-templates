...
if template_name in ('cluster_3nic') and 'experimental' in support_type:
    ...
    data['parameters']['internalLoadBalancerProbePort'] = {"defaultValue": "DO_NOT_USE", "metadata": { "description": "" }, "type": "string"}
    ...
    if stack_type == 'new_stack':
        ...
        if template_name in ('standalone_2nic', 'standalone_3nic', 'standalone_n-nic', 'cluster_3nic', 'ha-avset'):
            ...
            if template_name in ('ha-avset', 'cluster_3nic'):
                ...
                data['variables']['newStorageAccountName2'] = "[concat(uniqueString(resourceGroup().id, deployment().name), 'stor2')]"
                if template_name in ('cluster_3nic') and 'experimental' in support_type:
...
if template_name in ('ha-avset', 'cluster_3nic'):
    ...
    if template_name in ('ha-avset'):
        if stack_type in ('new_stack'):
            resources_list += [{ "apiVersion": api_version, "dependsOn": depends_on_ext + depends_on_ext_pub0, "location": location, "name": "[concat(variables('extNicName'), '0')]", "properties": { "copy": [ { "name": "ipConfigurations", "count": "[add(variables('numberOfExternalIps'), 1)]", "input": { "name": "[if(equals(copyIndex('ipConfigurations', 1), 1), concat(variables('instanceName'), '-self-ipconfig'), concat(variables('resourceGroupName'), '-ext-ipconfig', sub(copyIndex('ipConfigurations', 1), 2)))]", "properties": { "PublicIpAddress": { "Id": "[if(equals(copyIndex('ipConfigurations', 1), 1), concat(variables('extSelfPublicIpAddressIdPrefix'), '0'), concat(variables('extPublicIPAddressIdPrefix'), sub(copyIndex('ipConfigurations', 1), 2)))]" }, "primary": "[if(equals(copyIndex('ipConfigurations', 1), 1), 'True', 'False')]", "privateIPAddress": "[if(equals(copyIndex('ipConfigurations', 1), 1), variables('extSubnetPrivateAddress'), concat(variables('extSubnetPrivateAddressPrefix'), 1, sub(copyIndex('ipConfigurations', 1), 2)))]", "privateIPAllocationMethod": "Static", "subnet": { "id": "[variables('extSubnetId')]" } } } } ], "networkSecurityGroup": { "id": "[concat(variables('extNsgId'))]" } }, "tags": tags, "type": "Microsoft.Network/networkInterfaces" }]
        if stack_type in ('existing_stack', 'prod_stack'):
            resources_list += [{ "apiVersion": api_version, "dependsOn": depends_on_ext + depends_on_ext_pub0, "location": location, "name": "[concat(variables('extNicName'), '0')]", "properties": { "copy": [ { "name": "ipConfigurations", "count": "[add(variables('numberOfExternalIps'), 1)]", "input": { "name": "[if(equals(copyIndex('ipConfigurations', 1), 1), concat(variables('instanceName'), '-self-ipconfig'), concat(variables('resourceGroupName'), '-ext-ipconfig', sub(copyIndex('ipConfigurations', 1), 2)))]", "properties": { "PublicIpAddress": { "Id": "[if(equals(copyIndex('ipConfigurations', 1), 1), concat(variables('extSelfPublicIpAddressIdPrefix'), '0'), concat(variables('extPublicIPAddressIdPrefix'), sub(copyIndex('ipConfigurations', 1), 2)))]" }, "primary": "[if(equals(copyIndex('ipConfigurations', 1), 1), 'True', 'False')]", "privateIPAddress": "[if(equals(copyIndex('ipConfigurations', 1), 1), variables('extSubnetPrivateAddress'), concat(variables('extSubnetPrivateAddressPrefix'), add(variables('extSubnetPrivateAddressSuffixInt'), sub(copyIndex('ipConfigurations', 1), 1))))]", "privateIPAllocationMethod": "Static", "subnet": { "id": "[variables('extSubnetId')]" } } } } ], "networkSecurityGroup": { "id": "[concat(variables('extNsgId'))]" } }, "tags": tags, "type": "Microsoft.Network/networkInterfaces" }]
    ...
    if template_name in ('cluster_3nic') and 'experimental' in support_type:
        resources_list += [{"apiVersion": api_version, "dependsOn": depends_on_ext + depends_on_ext_pub0 + ["[variables('intLbId')]"], "location": location, "name": "[concat(variables('intNicName'), '0')]", "properties": { "primary": True, "enableIPForwarding": True, "ipConfigurations": [ { "name": "[concat(variables('dnsLabel'), '-int-ipconfig')]", "properties": { "primary": True, "privateIPAddress": "[variables('intSubnetPrivateAddress')]", "privateIPAllocationMethod": "Static", "subnet": { "id": "[variables('intSubnetId')]" } } }, { "name": "[concat(variables('dnsLabel'), '-int-ipconfig-secondary')]", "properties": { "privateIPAddress": "[variables('intSubnetPrivateAddress2')]", "privateIPAllocationMethod": "Static", "subnet": { "id": "[variables('intSubnetId')]" }, "loadBalancerBackendAddressPools": "[if(equals(parameters('internalLoadBalancerProbePort'), 'DO_NOT_USE'), take(variables('backEndAddressPoolArray'), 0), take(variables('backEndAddressPoolArray'), 2))]" } } ] }, "tags": tags, "type": "Microsoft.Network/networkInterfaces"}]
        resources_list += [{"apiVersion": api_version, "dependsOn": depends_on_ext + depends_on_ext_pub1 + ["[variables('intLbId')]"], "location": location, "name": "[concat(variables('intNicName'), '1')]", "properties": { "primary": True, "enableIPForwarding": True, "ipConfigurations": [ { "name": "[concat(variables('dnsLabel'), '-int-ipconfig')]", "properties": { "primary": True, "privateIPAddress": "[variables('intSubnetPrivateAddress1')]", "privateIPAllocationMethod": "Static", "subnet": { "id": "[variables('intSubnetId')]" } } }, { "name": "[concat(variables('dnsLabel'), '-int-ipconfig-secondary')]", "properties": { "privateIPAddress": "[variables('intSubnetPrivateAddress3')]", "privateIPAllocationMethod": "Static", "subnet": { "id": "[variables('intSubnetId')]" }, "loadBalancerBackendAddressPools": "[if(equals(parameters('internalLoadBalancerProbePort'), 'DO_NOT_USE'), take(variables('backEndAddressPoolArray'), 0), take(variables('backEndAddressPoolArray'), 2))]" } } ] }, "tags": tags, "type": "Microsoft.Network/networkInterfaces"}]
    else:
...
if template_name in ('ha-avset', 'cluster_3nic'):
    resources_list += [{ "type": "Microsoft.Storage/storageAccounts", "apiVersion": storage_api_version, "location": location, "name": "[variables('newStorageAccountName2')]", "tags": tags, "properties": { "accountType": "[variables('storageAccountType')]" } }]
...
if template_name in 'ha-avset':
    base_cmd_to_execute = "'mkdir -p /config/cloud/azure/node_modules && cp f5-cloud-libs*.tar.gz* /config/cloud; BIG_IP_CREDENTIALS_FILE=/config/cloud/.passwd; /usr/bin/install -b -m 755 /dev/null /config/verifyHash; /usr/bin/install -b -m 755 /dev/null /config/installCloudLibs.sh; /usr/bin/install -b -m 400 /dev/null $BIG_IP_CREDENTIALS_FILE; /usr/bin/install -b -m 400 /dev/null /config/cloud/.azCredentials; /usr/bin/install -b -m 755 /dev/null /config/cloud/managedRoutes; /usr/bin/install -b -m 755 /dev/null /config/cloud/routeTableTag;<BIGIQ_PWD_CMD> IFS=', variables('singleQuote'), '%', variables('singleQuote'), '; echo -e ', variables('verifyHash'), ' > /config/verifyHash; echo -e ', variables('installCloudLibs'), ' > /config/installCloudLibs.sh; echo -e ', variables('installCustomConfig'), ' >> /config/customConfig.sh; echo ', variables('singleQuote'), parameters('adminPassword'), variables('singleQuote'), '|sha512sum|cut -d \" \" -f 1 >> $BIG_IP_CREDENTIALS_FILE; echo ', variables('singleQuote'), '{\"clientId\": \"', parameters('clientId'), '\", \"tenantId\": \"', parameters('tenantId'), '\", \"secret\": \"', parameters('servicePrincipalSecret'), '\", \"subscriptionId\": \"', variables('subscriptionID'), '\", \"resourceGroup\": \"', variables('resourceGroupName'), '\"}', variables('singleQuote'), ' > /config/cloud/.azCredentials; echo -e ', parameters('managedRoutes'), ' > /config/cloud/managedRoutes; echo -e ', parameters('routeTableTag'), ' > /config/cloud/routeTableTag; unset IFS; bash /config/installCloudLibs.sh; bash /config/cloud/azure/node_modules/f5-cloud-libs/scripts/createUser.sh --user svc_user --password-file $BIG_IP_CREDENTIALS_FILE;<ANALYTICS_HASH> /usr/bin/f5-rest-node /config/cloud/azure/node_modules/f5-cloud-libs/scripts/onboard.js --output /var/log/onboard.log --log-level debug --host '"
