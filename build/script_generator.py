...
def script_creation(template_info, data, default_payg_bw, language):
    ...
    lic_type = lic_type_check(template_info['lic_support'][template_name])
    lic_count = lic_count_check(template_info['lic_key_count'][template_name])
    ...
    if language == 'powershell':
        ...
        if lic_type is True:
            ...
            if lic_count is True:
                byol_cmd = base_deploy + deploy_cmd_params + '\\"licenseKey1\\":{\\"value\\":\\"$licenseKey1\\"},\\"licenseKey2\\":{\\"value\\":\\"$licenseKey2\\"}}"'
                payg_cmd = base_deploy + deploy_cmd_params + '\\"licensedBandwidth\\":{\\"value\\":\\"$licensedBandwidth\\"}}"'
            else:
                byol_cmd = base_deploy + deploy_cmd_params + '\\"licenseKey1\\":{\\"value\\":\\"$licenseKey1\\"}}"'
                payg_cmd = base_deploy + deploy_cmd_params + '\\"licensedBandwidth\\":{\\"value\\":\\"$licensedBandwidth\\"}}"'
            deploy_cmd = 'if [ $licenseType == "BYOL" ]; then\n    ' + byol_cmd + '\nelif [ $licenseType == "PAYG" ]; then\n    ' + payg_cmd + '\nelse\n    echo "Please select a valid license type of PAYG or BYOL."\n    exit 1\nfi'
        else:
