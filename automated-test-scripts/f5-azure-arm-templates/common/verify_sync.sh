...
"No")
    ...
    response=$(ssh -o "StrictHostKeyChecking no" -i $SSH_KEY dewpoint@${BASTION_HOST} "curl -ku dewpoint:'<AUTOFILL PASSWORD>' --connect-timeout 10 $HOST/mgmt/tm/cm/sync-status | jq .") ;;
*)
    response=$(curl -ku dewpoint:'<AUTOFILL PASSWORD>' --connect-timeout 10 $HOST/mgmt/tm/cm/sync-status | jq .) ;;
esac

if echo $response | grep 'high-availability' && echo $response | grep 'All devices in the device group are in sync'; then
    echo "SUCCESS"
fi
