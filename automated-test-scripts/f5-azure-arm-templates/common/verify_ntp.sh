...
if [ "<TOKEN AUTH>" = "Yes" ]; then
    ...
    response=$(curl -k -H "X-F5-Auth-Token: $token" --connect-timeout 10 $HOST/mgmt/tm/sys/ntp)
fi
...
"No")
    ...
    response=$(ssh -o "StrictHostKeyChecking no" -i $SSH_KEY dewpoint@${BASTION_HOST} "curl -ku dewpoint:'<AUTOFILL PASSWORD>' ${HOST}/mgmt/tm/sys/ntp") ;;
*)
    response=$(curl -ku dewpoint:'<AUTOFILL PASSWORD>' --connect-timeout 10 ${HOST}/mgmt/tm/sys/ntp) ;;
esac
