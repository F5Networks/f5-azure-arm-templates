...
if [[ "<DECLARATION URL>" == "NOT_SPECIFIED" ]]; then
    echo "SUCCESS"
fi
...
"No")
    ...
    response=$(ssh -o "StrictHostKeyChecking no" -i $SSH_KEY dewpoint@${BASTION_HOST} "curl -ku dewpoint:'<AUTOFILL PASSWORD>' ${HOST}/mgmt/shared/appsvcs/declare") ;;
*)
    response=$(curl -ku dewpoint:'<AUTOFILL PASSWORD>' --connect-timeout 10 ${HOST}/mgmt/shared/appsvcs/declare) ;;
esac
...
# evaluate response
if echo $response | grep 'Sample_02'; then
    echo "SUCCESS"
else
    echo "FAILED"
fi
