#  expectValue = "Phone home verified"
#  scriptTimeout = 2
#  replayEnabled = true
#  replayTimeout = 10
...
"No")
    ...
    response=$(ssh -o "StrictHostKeyChecking no" -i $SSH_KEY dewpoint@${BASTION_HOST} "curl -ku dewpoint:'<AUTOFILL PASSWORD>' ${HOST}/mgmt/tm/sys/software/update | jq .autoPhonehome") ;;
*)
    response=$(curl -ku dewpoint:'<AUTOFILL PASSWORD>' --connect-timeout 10 ${HOST}/mgmt/tm/sys/software/update | jq .autoPhonehome) ;;
esac

if echo $response | grep $phonehome ; then
    echo "Phone home verified"
fi