...
"No")
    ...
    response=$(ssh -o "StrictHostKeyChecking no" -i $SSH_KEY dewpoint@${BASTION_HOST} "curl -ku dewpoint:'<AUTOFILL PASSWORD>' ${HOST}/mgmt/tm/net/self") ;;
*)
    response=$(curl -ku dewpoint:'<AUTOFILL PASSWORD>' --connect-timeout 10 ${HOST}/mgmt/tm/net/self) ;;
esac

echo "Self IP Response: $response"

# evaluate response
if echo $response | grep 'self_'; then
    echo "SUCCESS"
else
    echo "FAILED"
fi
