...
"No")
    ...
    response=$(ssh -o "StrictHostKeyChecking no" -i $SSH_KEY dewpoint@${BASTION_HOST} "curl -ku dewpoint:'<AUTOFILL PASSWORD>' ${HOST}/mgmt/shared/cloud-failover/info") ;;
*)
    response=$(curl -ku dewpoint:'<AUTOFILL PASSWORD>' --connect-timeout 10 ${HOST}/mgmt/shared/cloud-failover/info) ;;
esac

echo "CFE Info: $response"

# Evaluate
if echo $response | jq -r | grep 'version'; then
    result="SUCCESS"
else
    result="FAILED"
fi
