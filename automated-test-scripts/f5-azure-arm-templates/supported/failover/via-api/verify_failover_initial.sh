#  expectValue = "IP is associated with device"
#  scriptTimeout = 5
#  replayEnabled = true
#  replayTimeout = 40
...
# Not checking route failover here since we are possibly updating the CFE config after initial failover is triggered
case <CREATE PUBLIC IP> in
"No")
    ...
    if echo $IPCONFIG_NAME | grep -i <RESOURCE GROUP>-ext-ipconfig0 && echo $PROVISIONING_STATE | grep -i "Succeeded"; then
